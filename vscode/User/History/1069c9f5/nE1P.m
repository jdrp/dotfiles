function BER = transceiverBlock(num, NFFT, firstCarrier, cpLength, m_ary, numCarriers, generatorPolys, scramblerSeed, constellation, h, snr)
    rng(40);
    % For each OFDM symbol and subcarrier, 1 to 3 bits according to the modulation order. If FEC, apply coding rate. 
    numBits = numSymbols * numCarriers * log2(m_ary) / max(1, size(generatorPolys, 1));
