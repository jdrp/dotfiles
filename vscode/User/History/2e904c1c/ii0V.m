function plotBER(modulators, demodulators, BER_teor, modulations, EbNo_dB, N)
    numBits = N;
    ...
    % Crear gráfico y leyenda dinámicos para cada modulación
    for i = 1:length(modulators)
        semilogy(EbNo_dB, BER_teor(i,:), 'b-', 'DisplayName', sprintf('%s Teórica', modulations{i}))
        semilogy(EbNo_dB, BER(i,:), 'bo', 'DisplayName', sprintf('%s Simulada', modulations{i}))
    end
