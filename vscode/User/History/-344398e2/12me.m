...
%}
    ...
    for m_index = 1:length(m_arys)
        % Plot simulated BER
        semilogy(snr_dB, simulated_BER(m_index, :)+1e-10, '-o', ...
            'Color', colors(m_index, :), ...
            'DisplayName', ['BER simulada, orden ' num2str(m_arys(m_index))]);

        % Plot theoretical BER
        semilogy(snr_dB, theoretical_BER(m_index, :), '--x', ...
            'Color', colors(m_index, :), ...
            'DisplayName', ['BER teórica, orden ' num2str(m_arys(m_index))]);
    end
