function plotBER(snr_dB, m_arys, simulated_BER, theoretical_BER, y_lines)
    ...
    h_line_sim = plot(NaN, NaN, '-o', 'Color', 'k', 'DisplayName', 'línea -o : BER simulada');
    h_line_theo = plot(NaN, NaN, '--x', 'Color', 'k', 'DisplayName', 'línea discontinua : BER teórica');
    ...
    for m_index = 1:length(m_arys)
        ...
        h_colors(m_index) = plot(NaN, NaN, '-', 'Color', colors(m_index, :), ...
            'LineWidth', 2, 'DisplayName', ['Color : orden ' num2str(m_arys(m_index))]);
    end
