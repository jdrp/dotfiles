...
% Generación de pocos símbolos OFDM
Nofdm  = 4;  % Número de símbolos OFDM
[txbits, x] = randomOFDMwithQPSK(NFFT, Nf, m_ary, Nofdm);
...
Ts = 1/Fs;
t = (Ts:Ts:Ts*length(x)).*1000;
plot(t, x)
xlabel('t (ms)');  
ylabel('Amplitud (V)')
title('Señal de cuatro símbolos OFDM')
...
title(['Potencia de la señal OFDM,  PAPR = ' num2str(round(papr_dB,1)), 'dB']);
ylim([0, Ppeak * 1.05])
hold off;



% Generación de más símbolos OFDM
Nofdm  = 100000;  % Número de símbolos OFDM
[txbits, x] = randomOFDMwithQPSK(NFFT, Nf, m_ary, Nofdm);
...
frec = (0:N/2-1)*(Fs/N); 	% Vector de frecuencias a representar
   ...
   xlabel('f (KHz)');   
   ylabel('Espectro (dB)');
   xlim([0, frec(end)*1e-3])
   ylim([min(XdB), max(XdB)*1.02])
   title('Espectro OFDM con 15dB de SNR')
