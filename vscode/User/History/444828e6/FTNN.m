% function result = applyFunction(inputFunction, x)
    % inputFunction: a function handle passed as an argument
    % x: the input value to the function

%    result = inputFunction(x);
%end

function modulaConRuido(bits, modulador, SNR_dB)
    numBits = length(bits);
