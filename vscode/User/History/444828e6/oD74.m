function modulaConRuido(bits, modulator, SNR_dB, modulation, M)
    ...
    for i = 1:numSymbols
        ...
        colors(i,:) = symbolColors(bit2int(bits((i-1)*bitsPerSymbol+1 : i*bitsPerSymbol)) + 1, :);
    end
    ...
    for k = 0:M-1
        scatter(NaN, NaN, 100, symbolColors(k + 1, :), 'filled', 'DisplayName', int2bit(k, bitsPerSymbol));
    end
