function [interleavedBits] = interleaver(bits, numSymbols)
    ...
    for i = 1:numSymbols
        ...
        wIndex = mod(k, s) * Ncbps / s + floor(k / s) + 1; % Ensure indices are integers
        wIndex
        interleavedSymbolBits = zeros(1, Ncbps);
        interleavedSymbolBits(wIndex) = symbolBits(k + 1); % Adjust indexing for MATLAB
