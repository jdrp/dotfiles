...
% Llenamos con ceros h[n] hasta longitud N
h_padded = [h; zeros(N - length(h), 1)];

% Calculamos su espectro H(k)
H = fftshift(fft(h_padded));

% Plot del espectro de h[n]
figure;
plot(f, abs(H));
xlabel("Frecuencia (Hz)");
ylabel("|H(k)|");
