...
% Plot in the same figure x and x_ext, and make the cyclic prefix different in color
% Vectors must be the same length. be careful!
size(x) %
size(x_ext)
figure;
plot(1:N, x, 1:N, x_ext, N+1:N+L, x_ext(N+1:N+L), "r");
xlabel("n");
ylabel("x[n]");
legend("x[n]", "x_{ext}[n]", "Prefijo Cíclico");
