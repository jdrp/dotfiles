function graficaBER(EbNo_dB, BER, BER_teor, modulations, plot_title)
    ...
    % generar colores distintos para cada modulacion
    colors = lines(length(modulations));

    for i = 1:length(modulations)
        if isempty(BER_teor) || size(BER_teor, 1) < i || isempty(BER_teor(i,:))
            % si solo hay simuladas
            semilogy(EbNo_dB, BER(i,:), 'o--', 'Color', colors(i,:), 'DisplayName', modulations{i});
        else
            % si tambien hay teoricas
